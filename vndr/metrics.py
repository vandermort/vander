...
class PrecisionAtK(object):
    ...
    def __call__(self, gold, logits):
        ...
        for g, l in zip(gold, logits):
            ...
            card = len(g_set)
            tp += len(top_k.intersection(g_set))
            total += min(self.k, card)

        if len(self.tp_buffer) == self.window_size:
            self.tp_buffer.pop()
            self.total_buffer.pop()
        self.tp_buffer.appendleft(tp)
        self.total_buffer.appendleft(total)
    ...
    def value(self):
        ...
        if length:
            ...
            sum_total = sum(self.total_buffer)
            prc = sum_tp / max(sum_total, 1)
            return prc
        else:
            return 0.
