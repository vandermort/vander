...
if __name__ == "__main__":
    ...
    df = df[(df.slack_dims == args.slack_dims)]
    print(df)
    ...
    ax.set_xlabel('# of Labels (n)', fontsize=25)
    ax.set_ylabel('1-Percentile of Chebyshev radius $\epsilon$', fontsize=25)
    ...
    if '_p_' in args.prop:
        ...
        if args.slack_dims:
            plt.suptitle('$\epsilon$-argmaxability of $k$-sparse label assignments (%d slack variables)' % args.slack_dims, fontsize=25)
        else:
            plt.suptitle('$\epsilon$-argmaxability of $k$-sparse label assignments (no slack variables)', fontsize=25)
    # ax.set_xlim([-1, len(d_slice.N)])
    plt.legend(fontsize=24)
    plt.tight_layout()
    if args.save:
        filename = '%s-%s-dim-%d-%s.png' % (args.data.split('.')[0], args.slack_dims, args.prop)
